...
class Home extends Component {
  ...
  componentDidMount=()=>{
    console.log("tokennnnnn  isssssss" ,localStorage.getItem('token'))
    Axios.get(`http://localhost:8080/jobonja_war_exploded/main/ProjectsController?number=${this.state.visible}&token=${"Bearer " + localStorage.getItem('token')}`
  ...
  //   fetch(
  //     `http://localhost:8080/jobonja_war_exploded/main/ProjectsController `,{
  //       method :'get',
  //       headers:new Headers({
  //         'authorization': "Bearer " + localStorage.getItem('token')
  //       }),
  //     body :`number=${this.state.visible}`
  //   })
  //   .then((response )=> {
    ...
    console.log(event.target.value);
        if(event.target.value ==null){
          ...
          //,{ headers: { authorization: "Bearer " + localStorage.getItem('token') }}
            ...
            Axios.get(`http://localhost:8080/jobonja_war_exploded/main/SearchInProjectsController?searchQuery=${event.target.value}&token=${"Bearer " + localStorage.getItem('token')}`
           // ,{ headers: { authorization: "Bearer " + localStorage.getItem('token') }}
          ).then((response )=> {
